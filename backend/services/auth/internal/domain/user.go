...
// ValidEmail performs lightweight email shape validation sufficient for
// signup-time rejection. Full deliverability checking is out of scope.
...
// ValidPassword checks length only.
func ValidPassword(p string) bool { return len(p) >= MinPasswordLength }

// DefaultName derives a display name from the email local part when none was
// supplied.
