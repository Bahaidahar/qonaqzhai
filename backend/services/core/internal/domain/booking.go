...
// Booking is a customer's reservation. CustomerID is a foreign UUID from
// auth-svc; VendorID is local.
type Booking struct {
	ID         string        `json:"id"`
...
// VendorMayTransition reports whether a vendor may move the booking from its
// current status to the requested next status.
...
// CustomerMayTransition reports whether the booking's customer may move the
// booking to the requested status.
