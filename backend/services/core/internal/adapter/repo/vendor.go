...
func NewVendorRepo(db *sql.DB, idGen ports.IDGen) *VendorRepo {
	return &VendorRepo{db: db, idGen: idGen}
}

const vendorCols = `id, user_id, name, category, city, description, price_from, status, rating_avg, rating_count, created_at, updated_at`
...
// FindByID returns the vendor by primary key.
...
// FindByIDs returns vendors matching the supplied ids.
func (r *VendorRepo) FindByIDs(ctx context.Context, ids []string) ([]*domain.Vendor, error) {
	if len(ids) == 0 {
		return nil, nil
	}
	rows, err := r.db.QueryContext(ctx,
		`SELECT `+vendorCols+` FROM vendors WHERE id = ANY($1)`, ids,
	)
	if err != nil {
		return nil, fmt.Errorf("vendors by ids: %w", err)
	}
	defer rows.Close()
	return r.scanWithPhotos(ctx, rows)
}

// Search runs the catalog query with filters + pagination.
func (r *VendorRepo) Search(ctx context.Context, q ports.VendorQuery) ([]*domain.Vendor, int, error) {
	var wheres []string
	var args []any
	...
	var total int
	if err := r.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM vendors v`+whereSQL, args...).Scan(&total); err != nil {
	...
	tail := fmt.Sprintf(" LIMIT $%d OFFSET $%d", len(args)-1, len(args))
	rows, err := r.db.QueryContext(ctx,
		`SELECT `+vendorCols+` FROM vendors v`+whereSQL+orderBy+tail, args...,
	)
	if err != nil {
		return nil, 0, fmt.Errorf("list vendors: %w", err)
	}
	defer rows.Close()
	out, err := r.scanWithPhotos(ctx, rows)
	if err != nil {
		return nil, 0, err
	}
...
// CountByStatus returns counts grouped by status (one query).
func (r *VendorRepo) CountByStatus(ctx context.Context) (map[domain.VendorStatus]int, error) {
	rows, err := r.db.QueryContext(ctx, `SELECT status, COUNT(*) FROM vendors GROUP BY status`)
	if err != nil {
	...
	for rows.Next() {
		...
		out[domain.VendorStatus(status)] = n
	}
	return out, rows.Err()
}
...
func (r *VendorRepo) queryVendor(ctx context.Context, where string, args ...any) (*domain.Vendor, error) {
	row := r.db.QueryRowContext(ctx,
		`SELECT `+vendorCols+` FROM vendors `+where, args...,
	)
...
func (r *VendorRepo) scanWithPhotos(ctx context.Context, rows *sql.Rows) ([]*domain.Vendor, error) {
	out := []*domain.Vendor{}
	for rows.Next() {
