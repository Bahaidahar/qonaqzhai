// Package ws hosts the WebSocket hub used for client push.
package ws
...
func NewHub(logger *slog.Logger) *Hub {
	...
	return &Hub{clients: map[string]map[*Conn]struct{}{}, logger: logger}
}

// Attach binds the upgraded ws to userID and runs the pumps until the peer
// disconnects.
...
// Publish broadcasts payload to every active connection of each userID.
func (h *Hub) Publish(event string, payload []byte, userIDs ...string) {
	raw, err := json.Marshal(Envelope{Op: event, Data: payload})
	if err != nil {
