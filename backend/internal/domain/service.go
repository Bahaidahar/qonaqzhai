...
const (
	UnitFixed  ServiceUnit = "fixed"  // one-off package price
	UnitHour   ServiceUnit = "hour"   // per hour
	UnitItem   ServiceUnit = "item"   // per item/piece
	UnitPerson ServiceUnit = "person" // per guest / participant
	UnitDay    ServiceUnit = "day"    // per day
)
...
// Service is one offering on a vendor's menu (e.g. "Wedding cake", "DJ hour",
// "Ballroom rental"). A vendor may publish many services.
type Service struct {
	ID          string      `json:"id"`
